...
public class FenceHouseServiceImpl implements FenceHouseService {
    ...
    public ManagerFenceHouseExt findById(String fhId){
        ManagerFenceHouseExt houseExt = houseExtMapper.selectHouseAndHurdlesById(fhId);
        if(houseExt == null){
...
//b.判断fhName是否有效[唯一 或 原值]
            ...
            if(isExist(fhName) && !fhName.equals(houseDB.getFhName()))
                throw new ServiceException(ResultCode.FENCE_HOUSENAME_IS_EXIST);
//c.更新操作
            updateCount = houseMapper.updateByPrimaryKey(house);
        } else {
//2.2 新增
//a.栏舍名称 唯一校验
            if(isExist(house.getFhName()))
                throw new ServiceException(ResultCode.FENCE_HOUSENAME_IS_EXIST);
//b.自动生成id值
