...
//3.组合map集合对象并返回
    ...
    public Map<String, List<Object>> countSales(){
        //1.准备月份列表
        List<Object> months = Arrays.asList("1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12 月");
        //2.准备对应销售总数列表，默认销量为0
...
//直接转换服务器会报错：java.math.BigDecimal cannot be castto java.lang.Integer
            int count = new BigDecimal(map.get("销售总数")+"").intValue();
...
*/
        List<Map<String, Object>> mapList = diseaseMapper.countDisease();
...
// });
    ...
    public Map<String, Integer> countWeight() {
        Map<String, Integer> map = animalMapper.countWeight();
        return map;
    }
}
